...
public class MessageRepository (AppDbContext context): IMessageRepository
{
    ...
    {
        return await context.Groups.Include(x => x.Connections.Any(c => c.ConnectionId == connectionId)).FirstOrDefaultAsync();
    }

    public async Task<Message?> GetMessage(string messageId)
    {
        return await context.Messages.FirstOrDefaultAsync(x => x.Id == messageId);
        // return await context.Messages.FindAsync(x => x.Id == messageId);
    }
    ...
    public async Task<PaginatedResult<MessageDto>> GetMessagesForMember(MessageParams messageParams)
    {
        ...
        return await PaginationHelper.CreatesAsync(messageQuery, messageParams.PageNumber, messageParams.PageSize);
    }

    public async Task<IReadOnlyList<MessageDto>> GetMessageThread(string currentMemberId, string recipientId)
    {
        //timestamp for read messages
        ...
        return await context.Messages
                        .Where(
                            x => (x.RecipientId == currentMemberId && x.RecipientDeleted==false && x.SenderId == recipientId)
                            ||
